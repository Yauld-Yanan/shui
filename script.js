class Watermarker {
    ...
    addWatermark() {
        ...
        // 计算水印大小比例（根据图片宽度调整）
        const scaleFactor = Math.max(this.canvas.width / 1000, 1); // 基准宽度1000px
        
        // 设置字体样式
        this.ctx.fillStyle = 'rgba(255, 255, 255, 0.7)';
        
        // 计算字体大小
        const timeFontSize = Math.round(80 * scaleFactor);
        const normalFontSize = Math.round(32 * scaleFactor);
        
        // 计算边距
        const bottomMargin = 60 * scaleFactor;
        const leftMargin = 50 * scaleFactor;
        ...
        this.ctx.fillText(dateStr, lineX + (20 * scaleFactor), this.canvas.height - bottomMargin - (30 * scaleFactor));
        this.ctx.fillText(weekdayStr, lineX + (20 * scaleFactor), this.canvas.height - bottomMargin);
